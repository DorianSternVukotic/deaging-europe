...
**Brand tokens** — `src/styles/global.css`: `--color-bone/ink/sage/rose/gold`, `--font-display` (Cormorant Garamond) and `--font-sans` (Inter), loaded from Google Fonts in `BaseLayout`. Evidence-tier colours are the saturated `evidenceColor` set in `src/data/evidence.ts`, not the brand tones. `html { scroll-behavior: smooth }` is global — scroll with `behavior: 'instant'` when scripting screenshots. **Angular language (since 2026-09-11):** no `rounded-*` utilities or `border-radius` anywhere, no drop shadows; hover/open feedback is border colour only (`hover:border-rose`), outline buttons use a 1px border, and the desktop evidence bar in `EvidenceRow` (192×24, unchanged size) is a square fill on a four-segment ruled track: a hairline mark per tier drawn as 6px notches along the top and bottom edges (`.scale-track::before/::after`), over fill and track alike, with the tier word inside the fill ("limited" sits just after its 25% fill, which is too short for the word). The blurred rose blob behind hero sections was removed with it.
...
- `MediaPlaceholder` slots are intentional placeholders for images/video that don't exist yet — keep them; humans fill media later. `/collagen` is fully photographed (Pexels hotlinks via the `pexels()` helper in `collagen.ts` for section rows, page slots in `collagen.astro`); the other guides still show placeholders. **Aeterna's own photography** (supplied 2026-09-04 via the partner's Drive folder "deage.eu files") lives in `public/images/aeterna/` (resized JPEGs) and `public/video/` (an 18 s muted 720p how-to clip); it is the `/collagen` hero + og:image (the white-coat portrait the partner calls "the doctor picture"; name and credentials unknown), the `/collagen` video slot under the takeaways (the partner's two-women thumbnail with a baked-in play button; `media.video.src` in `collagen.astro` is empty until the clip arrives, so it renders as a still and becomes a `<video>` once set), the whole product gallery (five photographs since 2026-09-04, the three shop cut-outs were removed at the partner's request; the homepage card still uses the neutral cut-out via the product's `image:` field) and the how-to media. Static files under `public/images` and `public/video` are served by Wix as-is (`_routes.json` excludes them from the worker).
- The product page (`/products/collagen-max-pro`) was rebuilt 2026-09-03 from Aeterna's real product data: structured fields in the Markdown (variants, actives graded with the evidence tiers, label facts, studies, buyer quotes copied from aeterna.bg, FAQ) rendered by the `Product*` components. Three per-serving amounts (Dermial, Ceramosides, vitamin C) are not published online and are marked "on the tub label" until someone transcribes a pack. Gallery entries take `fit: contain` (default, cut-outs padded on the mint ground) or `fit: cover` (photographs fill the 4:5 frame); `howTo.video` (`src`, `poster`, `alt`) adds the looping clip beside the steps. Guide treatment sections are still partly uncited and their images are stock — see the `content-status-audit` memory.

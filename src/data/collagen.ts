...
/** The neutral tub cut-out — the same file as the product's `image:` field (Wix-hosted). */
export const PRODUCT_IMAGE =
  'https://static.wixstatic.com/media/51aaad_cf505ae35a114265a3db9a21f02b25f2~mv2.png/v1/fit/w_600,h_600,q_90/file.png';
...
const collagenSupplements: Section[] = [
  {
    ...
    bodyHtml: `
      ...
      <div class="product-pick mt-6 border border-accent/30 bg-accent/5 grid grid-cols-1 sm:grid-cols-[168px_minmax(0,1fr)] items-stretch">
        <a href="${PRODUCT_PATH}" class="product-ground block h-44 sm:h-auto" aria-label="Aeterna COLLAGEN MAX PRO — see the product"><img src="${PRODUCT_IMAGE}" alt="A tub of Aeterna COLLAGEN MAX PRO" loading="lazy" width="466" height="600" class="w-full h-full object-contain p-3 sm:p-4"></a>
        <div class="p-4 sm:p-5 min-w-0">
          <p class="text-[10px] uppercase tracking-[0.25em] text-accent">Our pick</p>
